...
using CalcSustain.WEB.Results;

namespace CalcSustain.WEB.Controllers
{
    /// <summary>
    /// контроллер, который содержит ряд методов и функциональностей для управления учетными записями (регистрация, вход на сайт и т.д.)
    /// </summary>
    [Authorize]
    ...
    public class AccountController : ApiController
    {
        ...
        /// <summary>
        /// маршрут, по которому будет перенаправляться пользователь для авторизации (set at Startup.Auth.cs)
        /// </summary>
        ...
        {
            // Если переданные данные проходят валидацию, то методом UserManager.CreateAsync() создается 
			// и добавляется в базу данных новый пользователь.
            //
            ...
            if (result.Succeeded)
            {
                // если создание прошло успешно, то включаем пользователя в роль user 
